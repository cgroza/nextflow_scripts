...
if(params.peak_call) {
    ...
    process callPeaksRef{
        ...
        input:
        set file(fastq), file("json"), val(control_name), file("control_json"), file("graphs") from ref_treatment_json_ch.combine(ref_control_json_ch).filter{design[it.get(0).getName()] == design[it.get(0).getName()]}.combine(ref_peak_linear_ch).map{ it.flatten()}.view()
